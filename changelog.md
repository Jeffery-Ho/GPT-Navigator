...
- 修复：设置菜单里的“同步设置”和“重置配置”按钮在浅色菜单背景下过浅，改为深色文字和边框。
- 版本：修复更新到 `0.3.1 build 10`。
- 需求：在设置菜单中加入“同步设置”操作。
- 实现：新增“同步设置”按钮，点击后手动从后台读取 `chrome.storage.sync` 云端配置并立即应用到当前页面。
- 说明：浏览器扩展不能执行本机 `git pull`；源码更新仍需在终端拉取后重新加载扩展。
- 版本：功能更新到 `0.3.0 build 9`。
- 需求：重做插件设置的多端同步配置能力。
- 实现：新增共享配置模块和 MV3 后台脚本，由后台统一读写 `chrome.storage.sync` 并向已打开页面广播配置变化。
- 实现：内容脚本不再直接访问同步存储，改为通过后台消息加载、保存和接收配置变更。
- 实现：Manifest 增加固定公钥，让不同设备加载同一份源码时使用稳定扩展 ID；仓库只提交公钥，不提交私钥。
- 保留：同步区为空时继续迁移当前站点旧 `localStorage` 配置，且不删除旧值。
- 版本：功能更新到 `0.2.0 build 8`。
